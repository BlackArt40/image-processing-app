...
基于 **Spring Boot + OpenCV** 的 Web 图片处理应用，提供六个功能模块，每个模块均支持**上传 → 异步处理 → 前后对比预览 → 一键下载**：

| 模块 | 说明 | 实现要点 |
| --- | --- | --- |
| 🖼️ **高清增强** | 超分辨率放大并锐化细节，**多算法可选**；经典管线可调 | 本机深度超分 **EDSR / FSRCNN / ESPCN / LapSRN**（2/3/4×）+ CLAHE 对比度 + Unsharp 锐化；或经典管线（**逐级 Lanczos 上采样** + CLAHE + 锐化）+ 去噪/对比度/锐化三相强度可调 |
...
| ☀️ **去雾 / 低光** | 去灰霾提通透，或对欠曝照片做亮度补正 | 暗通道先验（He）去雾 + LAB 亮度通道 gamma 低光增强，两项均可调强度 |
...
```json
// 高清增强（本机 AI 超分，可选算法；clarity/sharpn 作用于超分后处理）
{"type":"ENHANCE","scale":2,"superResAlgorithm":"lapsrn","clarity":50,"sharpen":50}
...
// 去雾 / 低光（0=关闭，默认 50，可单独或叠加）
{"type":"DEHAZE","dehaze":60,"lowLight":40}
```
...
├── dto/           # 请求与响应对象
├── processor/     # 七大处理器（增强、精修、去马赛克、转换、滤镜、去雾/低光）+ OpenCV 工具 + 人脸检测
